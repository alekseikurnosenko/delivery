...
export SPRING_RABBITMQ_USERNAME=$(echo $MSG_TMP_USER_PASS | cut -d':' -f1)
export SPRING_RABBITMQ_PASSWORD=$(echo $MSG_TMP_USER_PASS | cut -d':' -f2)

MSG_TMP_HOST_VHOST=$(echo $MSG_TMP | cut -d'@' -f2-)
export SPRING_RABBITMQ_HOST=$(echo $MSG_TMP_HOST_VHOST | cut -d'/' -f1)
export SPRING_RABBITMQ_VIRTUAL_HOST=$(echo $MSG_TMP_HOST_VHOST | cut -d'/' -f2)
export SPRING_RABBITMQ_PORT=5672

echo $SPRING_RABBITMQ_HOST